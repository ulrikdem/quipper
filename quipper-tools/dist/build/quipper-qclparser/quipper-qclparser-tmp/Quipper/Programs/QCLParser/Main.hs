...
{-# LINE 1 "Quipper/Programs/QCLParser/Main.hs" #-}
...
module Quipper.Programs.QCLParser.Main where
...
usage = do
  ...
  putStr (header name)
    where header name =
            name ++ ": read an execution trace produced by Ömer's QCL language,\n"
...
main = do
  ...
  print_simple ASCII (run (run_qcl_lines lines))
