...
{-# LINE 1 "Quipper/Programs/Tools/Strict.hs" #-}
-- ----------------------------------------------------------------------
-- | This tool decomposes any circuit into the gate base {/H/, /S/,
-- /T/, /CNot/}, using both exact and approximate synthesis.
...
usage = do
  ...
  putStr (usageInfo (header name) options)  
    where header name =
            name ++ ": decompose any circuit into the gate base {H, S, T, CNOT},\n"
...
main = do
  ...
  let decomposed_circuit = decompose_generic (Strict p (RandomSource g)) circuit
