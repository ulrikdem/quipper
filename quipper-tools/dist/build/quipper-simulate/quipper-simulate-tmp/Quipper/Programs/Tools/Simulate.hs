...
{-# LINE 1 "Quipper/Programs/Tools/Simulate.hs" #-}
...
import Quipper.Utils.Auxiliary (string_of_list)
...
usage = do
  ...
  putStr (usageInfo (header name) options)  
    where header name =
            name ++ ": reads a circuit from standard input, and then simulates\n"
            ++ "it by applying it to every possible basis vector. This is only\n"
            ++ "intended for debugging (of very small circuits), and is by no means\n"
            ++ "an industrial-strength simulator. Any measurements in the circuit\n"
            ++ "will be simulated probabilistically.\n\n"
...
-- simulation.
sim_amps_addprec :: forall g p qa qb.(RandomGen g, Precision p, QData qa, QData qb, Ord (BType qb)) => g -> Int -> (qa -> Circ qb) -> Map (BType qa) (Cplx (FixedPrec p)) -> Map (BType qb) (Cplx (FixedPrec p))
sim_amps_addprec g addprec circ state =
  with_added_digits addprec f (dummy state) :: Map (BType qb) (Cplx (FixedPrec p)) where
    dummy :: Map a (Cplx f) -> f
    dummy = undefined
    f :: (Precision e) => FixedPrec e -> Map (BType qb) (Cplx (FixedPrec p))
