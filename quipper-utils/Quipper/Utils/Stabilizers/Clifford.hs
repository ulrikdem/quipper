...
module Quipper.Utils.Stabilizers.Clifford where

import Prelude hiding (lookup,negate)
import Quipper.Utils.Stabilizers.Pauli
...
measure q tab = case anticommute_with_z of
  ...
  [(_,q_row)] -> do -- exaclty one anti-commutes, measurement result is 50/50
    let tab' = foldl' (\tab r -> if lookup (r,q) (de_tableau tab) `elem` [X,Y] then multiply_de r q_row tab else tab) tab [0..(nq-1)]
    let de_t = de_tableau tab'
    let de_s = de_sign tab'
...
multiply_de q_row1 q_row2 tab = ST nq s t s_de' t_de'
 where
  nq = next_qubit tab
  t = tableau tab
  s = sign tab
  t_de = de_tableau tab
  s_de = de_sign tab
  sign1 = lookup q_row1 s_de
  sign2 = lookup q_row2 s
  sp = One (multiply_sign sign1 sign2)
  (t_de',sp') = foldl' mul_col (t_de,sp) [0..(nq-1)] 
  s_de' = Map.insert q_row1 (signPlus_to_sign sp') s_de
  mul_col :: (Map (Qubit,Qubit) Pauli, SignPlus) -> Qubit -> (Map (Qubit,Qubit) Pauli, SignPlus)
  mul_col (tab_de,sp) q_col = (Map.insert (q_row1,q_col) p' tab_de,multiply_signPlus sp sp')
    where
     p1 = lookup (q_row1,q_col) tab_de
     p2 = lookup (q_row2,q_col) t
     (sp',p') = commute p1 p2
